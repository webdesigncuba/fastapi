...
import sysconfig
import tempfile
import certifi  # Agregado como sugerencia de para instalar en windows fuente: https://github.com/python-poetry/poetry/issues/680
...
def data_dir() -> Path:
    ...
    if WINDOWS:
        ...
        base_dir = Path(os.getenv("XDG_DATA_HOME",
    ...
    def make(cls, target: Path) -> "VirtualEnvironment":
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="poetry-installer") as temp_dir:
                ...
                request = Request(
                    virtualenv_bootstrap_url, headers={
                        "User-Agent": "Python Poetry"}
                )
...
def main():
    ...
    installer = Installer(
        ...
        preview=args.preview or string_to_bool(
            os.getenv("POETRY_PREVIEW", "0")),
        force=args.force,
...
if __name__ == "__main__":
    # Agregado para Instalar certificados que dan error en Windows
